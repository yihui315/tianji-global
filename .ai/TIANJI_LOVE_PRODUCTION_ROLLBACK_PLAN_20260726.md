...
>
> **2026-07-26 §11 correction (TASK_ID=TIANJI-PR183-SSH-EVIDENCE-CORRECTION-004):** SSH evidence reclassified as `CONFLICTING` (`SSH_EVIDENCE_CONFIDENCE=LOW`, `POLICY_DEVIATION=YES`). Two conflicting reports about the same execution environment now preserved in full (see READINESS_REVIEW §11.1). Historical Windows SSH record (`186.244.244.81`/`tianji-prod`/`ser8221021417`) is NOT the current Mac's config. Rollback anchor remains `UNKNOWN`; `SSH_ACCESS=BLOCKED`; abort any deploy attempt. See READINESS_REVIEW §11 for full audit.
