...
- Files changed: pricing/product configuration, canonical redirects and sitemap configuration, consent controls, love-reading CTA, brand metadata, legal copy, legacy-route indexing headers, AdSense audit/tests, CI and manual deployment release gates, `.ai/CHANGELOG_AI.md`, and `.ai/REVIEW_PACKET.md`.
- Summary: Implemented the source-code items from the final AdSense audit: centralized public prices, removed the coming-soon report from structured offers, consolidated duplicate locale routes through permanent redirects, added default-denied consent controls with withdrawal, corrected the love-reading CTA, removed stale brand/year signals, added noindex headers for legacy product routes, and made the source AdSense audit part of `release:check`. The follow-up review split consent authority so the first-party panel controls analytics only and never grants Google advertising signals. The manual production workflow now requires an exact 40-character `main` SHA, injects version metadata before build/restart, and runs the SHA-aware live AdSense audit after smoke. The generic CI workflow contains no production deployment job.
...
- Follow-up validation: workflow YAML parsing, focused AdSense contract tests, typecheck, and the full `npm run release:check` passed (83 files / 643 tests; 75 static pages). The initial focused run exposed and then closed a v2 consent-record migration typing/test mismatch. GitHub Node 20 clean-install CI remains required and will be obtained through the PR; local Node 24 `npm ci` remains an explicitly documented limitation.
- Follow-up safety: environment scope stayed local/source-only. No workflow dispatch, production deployment, Google CMP mutation, Stripe/Supabase call, `.env*` read, secret read, or paid smoke occurred. Production and AdSense submission remain `NO-GO`.
