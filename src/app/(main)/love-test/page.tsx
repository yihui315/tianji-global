...
export default function LoveTestPage() {
  ...
  const copyStatusText =
    copyState === 'copied'
      ...
      : copyState === 'error'
        ? 'Copy failed. You can still select the result text manually.'
        : '';
  ...
  const downloadShareCard = async (format: LoveTestShareFormat) => {
    ...
    try {
      const text = [
        `TianJi Free Fate Match Test - ${SHARE_FORMAT_LABELS[format]}`,
        `${payload.archetype}: ${payload.score}/100`,
        payload.oneLiner,
        `Try yours: ${payload.shareUrl}`,
      ].join('\n');
      await navigator.clipboard.writeText(text);
      setDownloadError(`${SHARE_FORMAT_LABELS[format]} share text copied without private inputs.`);
      void trackRevenueFunnelEvent('relationship_free_result_view', {
        source: 'love_test',
        surface: 'free_fate_match_share_text',
        format,
      ...
      setDownloadError('Share text could not be copied. Please copy the text result instead.');
    } finally {
      ...
      <section className="relative z-10 mx-auto grid w-full max-w-7xl gap-8 px-5 pb-10 pt-14 sm:px-8 lg:min-h-[650px] lg:grid-cols-[minmax(0,0.92fr)_minmax(420px,0.78fr)] lg:items-center">
        <div className="max-w-3xl">
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-3">
            ...
            <TianjiLoveTrustCard icon={Share2} title="Growth-ready" body="Copy privacy-safe social text for Xiaohongshu, Douyin, and Moments." />
          </div>
        </div>
        ...
        <section id="love-test-result" className="relative z-10 mx-auto w-full max-w-6xl px-5 pb-14 sm:px-8">
          ...
          <TianjiLovePanel className="mt-5 p-6">
            <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.24em] text-[#d8b77b]/62">Share formats</p>
                <h2 className="mt-2 font-serif text-2xl text-[#ffe3b4]">Copy private social text</h2>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-[#f4d7a3]/62">
                  Each format uses only score, archetype, one-line result, and a safe result URL.
                </p>
              </div>
              ...
              {LOVE_TEST_SHARE_FORMATS.map((format) => (
                ...
                >
                  <span className="flex items-center gap-2 text-sm font-semibold text-[#ffe3b4]">
                    <Share2 className="h-4 w-4 text-[#d8b77b]" aria-hidden />
                    Copy share text
                  </span>
                  <span className="mt-2 block text-sm text-[#f4d7a3]/66">{SHARE_FORMAT_LABELS[format]}</span>
                  <span className="mt-1 block text-xs text-[#f4d7a3]/44">{downloadState === format ? 'Copying...' : 'Ready'}</span>
                </button>
              ))}
            </div>
            <p role="status" aria-live="polite" className="mt-3 min-h-5 text-xs leading-5 text-[#f4d7a3]/58">
