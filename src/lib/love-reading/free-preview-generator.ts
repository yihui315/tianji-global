...
function uncertaintyNote(input: FreePreviewReportInput, locale: LoveReportLocale) {
  ...
  const english = onlyOne
    ? 'Only one profile is available, so this preview reflects personal relationship tendency rather than full compatibility.'
    : missingTime
      ? 'Precise timing data is missing, so this preview avoids exact timing claims.'
      : 'Location context is missing, so this preview uses conservative location-independent guidance.';

  const copy: Record<LoveReportLocale, string> = {
    en: english,
    zh: english,
    'zh-Hant': english,
  };
...
function copyFor(locale: LoveReportLocale) {
  const english = {
    ...
    teaser: 'Open the complete report for five-dimension depth, a 30-day window, and a repair script.',
    shareCta: 'Start your private TianJi Love preview',
  };

  const copy: Record<LoveReportLocale, typeof english> = {
    en: english,
    zh: english,
    'zh-Hant': english,
  };
