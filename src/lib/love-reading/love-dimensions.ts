...
const baseCopy: Record<LoveDimensionKey, Copy> = {
...
function copyForLocale(_locale: LoveReportLocale) {
  return baseCopy;
}
...
export function buildLoveDimensions(
  ...
  return LOVE_DIMENSION_KEYS.map((key) => {
    ...
    return {
      ...
      evidence: [
        'Deterministic relationship preview signal',
        score >= 70 ? 'Strong relative score band' : 'Needs mindful calibration',
      ],
      action: item.action,
      uncertaintyNote,
    };
